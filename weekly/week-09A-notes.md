...
- "The Firebase Realtime Database is a cloud-hosted NoSQL database that lets you store and sync data between your users in realtime."
  ...
  - "NoSQL" in this case means that all of your data is stored in what is essnetial an single JSON object, as opposed to being broken out into seperate SQL tables (ex.  `Person` and `Address` or `BlogPost`)
...
- Even though Firebase is NoSQL, our app still needs Firebase to do the same things that a SQL database does - pecifically the **CRUD** operations:
  ...
  - ***U*pdate** - if a user favorites a dog image that has already been favorited by someone else, 
  - ***D*elete**
